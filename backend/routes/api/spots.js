...
router.get("", async (req, res) => {
  ...
  if (size >= 1 && size <= 20) {
    ...
    errors.page =
      "Page must be greater than or equal to 1 and less than or equal to 20";
  }
  if (page >= 1 && page <= 10) {
    query.offset = size * (page - 1);
  } else {
    errors.size =
      "Size must be greater than or equal to 1 and less than or equal to 10";
  }
...
router.post("/:spotId/bookings", requireAuth, async (req, res) => {
  ...
  //Bring up all current bookings for spot
  const bookings = await Booking.findAll({
    where: {
      spotId: req.params.spotId,
    },
...
router.delete("/:spotId", requireAuth, async (req, res) => {
  ...
  await spot.destroy(); //added await for testing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
